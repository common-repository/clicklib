...
class phmagick_decorations{
    function roundCorners(phmagick $p,$i = 15, $color="#fff"){
    ...
    function dropShadow(phmagick $p,$color = '#000', $background = '#fff', $offset = 4, $transparency = 60, $top = 4, $left=4){
        ...
        $cmd .= " -background '$background' -mosaic ";
        $cmd .= ' "' . $p->getDestination().'"'  ;
     ...
     */
    function fakePolaroid(phmagick $p,$rotate = 0 , $borderColor = "#fff", $background ="none"){
        $cmd = $p->getBinary('convert');
        $cmd .= ' "' . $p->getSource().'"'  ;
        $cmd .= ' -bordercolor "'. $borderColor.'"  -border 6 -bordercolor grey60 -border 1 -background  "none"   -rotate '. $rotate .' -background  black   +swap -background  "'. $background.'"   -flatten';
        $cmd .= ' ' . $p->getDestination() ;
     ...
     * @param $background - Image background color (use for jpegs or images that do not support transparency or you will end up with a black background)
     */
    function polaroid(phmagick $p, $format = null, $rotation= 0, $borderColor="snow", $shaddowColor = "black", $background="none"){
