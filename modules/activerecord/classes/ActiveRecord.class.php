...
class ActiveRecord 
{
  ...
  function event($event_name, $event_args = array())
  {
    ...
    return Click::event($event_name, $event_args);
  }
  ...
  static function _select_assoc($klass, $params)
  {
    ...
    $data = Click::event($event_name, array('ar_params'=>$params));
  ...
  {
  	$klass=$this->klass;
  	$php = "return $klass::find_by_id(\$this->id);";
  	$obj = eval($php);
...
function index()
{
  ...
  function save($create_new = false)
  {
    ...
    foreach($an as $field)
      ...
      if (array_key_exists('db_format', $ms) && array_key_exists($field, $ms['db_format']))
      {
      	...
      	{
          click_error("{$klass}->{$field} is not of the format {$ms['db_format'][$field]}. Failed to serialize properly.", array($this, $event_table));
      	}
      }
    }
  ...
  function insert()
  {
  	...
  	if ($ms['is_auto_increment']['id']) 
  	{
  	  $this->id = mysql_insert_id(Click::$meta['db']['current']['handle']);
  	} else {
  		if ($attrs['id'])
  		{
  		  $id = $attrs['id'];
