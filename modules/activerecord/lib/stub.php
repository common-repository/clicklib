...
function call_ar_func($o, $name, $arguments)
{
    ...
    foreach(Click::$meta['plugins'] as $plugin_name=>$plugin_info)
    {
      foreach($plugin_info['modules'] as $module_name=>$module_info)
      {
        $model_fpath = $plugin_info['fpath']."/modules/{$module_name}/models";
        $extension_fpath = $model_fpath."/{$o->tableized_klass}.php";
